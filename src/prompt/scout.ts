...
1. 有一个符合人群特点的网络用户名
2. 3-5个多维度特征标签
3. 完整prompt(以"你是"开头，结尾强调个性)，至少500字的极其详细描述
4. 包含用户在不同平台的行为差异和典型表达方式
5. 明确说明用户偏好的平台及其原因

savePersona工具是最终输出的核心，必须包含非常详细的prompt描述（至少500字），这是最终用户将看到的唯一内容。使用savePersona工具保存每个完成的persona，不要等到全部完成再保存。
...
4. 为每种用户类型创建详细persona并使用savePersona工具保存
   ...
   - savePersona中的prompt必须极其详细（至少500字），这是最终用户看到的唯一输出
   - 包含具体的语言表达方式、行为模式和价值观
