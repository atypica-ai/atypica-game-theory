...
如果用户选择第一个选项，后续调用 searchPersonas 时必须将 privateOnly 设置为 true。
...
2. 【步骤2】使用 searchPersonas 工具查找现有用户画像智能体：
   ...
   • 【私有画像优先】如果用户选择优先使用私有真人画像，调用 searchPersonas 时必须将 privateOnly 参数设置为 true。后续步骤（如使用 scoutTaskChat + buildPersona）将用于补充数量不足的画像。
...
If the user chooses the first option, you must set privateOnly to true when calling searchPersonas.
...
2. 【Step 2】Use searchPersonas tool to find existing user persona AI Personas:
   ...
   • 【PRIVATE PERSONA PRIORITY】If the user chooses to prioritize their private personas, you must set the privateOnly parameter to true when calling searchPersonas. Subsequent steps (like using scoutTaskChat + buildPersona) will be used to supplement if not enough personas are found.
