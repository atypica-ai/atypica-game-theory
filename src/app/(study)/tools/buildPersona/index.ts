...
import { savePersonaTool } from "../savePersona";
import { scoutChatTools } from "../scoutTaskChat/types";
...
export async function runBuildPersona({
  ...
  const tools = {
    [StudyToolName.savePersona]: savePersonaTool({ scoutUserChatId }),
  ...
  const streamTextPromise = new Promise((resolve, reject) => {
    ...
    // const toolChoice: ToolChoice<typeof tools> = { type: "tool", toolName: StudyToolName.savePersona };
