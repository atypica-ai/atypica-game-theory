/**
 * 注意，这个 API 已经不用了，但是这里很多代码值得参考，只作为测试用途
 * ⚠️ 只有 admin 页面会用到这个路由
 * 不用这个路由，为了方便管理 CDN 的路由，现在统一和文件/CDN相关的，都用 /cdn/ 前缀的路由，比如 /cdn/proxy-image 等
 * 也是为了 ingress nginx 那里配置跳转规则简单点，cdn 路由是不能跳转的，不然会导致海外回源的时候经历重定向，遇到意想不到的问题
 */
