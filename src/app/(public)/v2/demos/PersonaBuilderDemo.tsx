...
export default function PersonaBuilderDemo() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => (procStep < 3 ? setProcStep((s) => s + 1) : setPhase("result")),
      procStep < 3 ? 800 : 400,
    );
  ...
  return (
    <ProductFrame
      ...
      breadcrumb={
        <>
          <BreadcrumbSegment text="Persona" />
          <BreadcrumbSeparator />
          <BreadcrumbSegment text="Import" active />
        </>
      }
    >
          ...
          <AnimatePresence mode="wait">
            ...
            {(phase === "home" || phase === "flying") && (
              ...
              >
                ...
                <p
                  className="font-IBMPlexMono text-xs tracking-wider uppercase mb-2"
                  style={{ color: L.textFaint }}
                >
                  Persona Platform
                </p>
                <h3 className="text-base font-medium mb-1.5" style={{ color: L.text }}>
                  Build AI Personas
                </h3>
                <p
                  className="text-xs leading-relaxed mb-5 max-w-[220px] mx-auto"
                  style={{ color: L.textMuted }}
                >
                  Import interview transcripts to create virtual consumers with real behavior
                  patterns.
                </p>
                ...
                <div className="grid grid-cols-2 gap-1.5 max-w-[240px] mx-auto">
                  {[
                    ...
                    <div
                      key={f.label}
                      className="py-2 px-2.5 rounded text-center"
                ...
                <div className="flex items-center gap-1.5">
                  {steps.map((label, i) => (
                    <div key={i} className="flex items-center gap-1.5 flex-1">
                      <div
                        className="w-6 h-6 rounded-full grid place-items-center text-xs font-IBMPlexMono shrink-0"
                        style={{
                          background:
                            i < procStep
                              ? `${L.green}20`
                      ...
                      <span
                        className="font-IBMPlexMono text-xs"
                        style={{ color: i <= procStep ? L.text : L.textFaint }}
                      >
                        {label}
                      </span>
                      {i < steps.length - 1 && (
                        <div
                          ...
                          style={{ background: i < procStep ? `${L.green}30` : L.border }}
                        />
                      )}
                    </div>
                  ))}
                ...
                <div className="space-y-2.5">
                  {[0.75, 0.5, 0.9, 0.4].map((w, i) => (
                    <motion.div
                      ...
                      style={{ background: L.bgSub, width: `${w * 100}%` }}
            ...
            {phase === "result" && (
              ...
              >
                ...
                <motion.div
                  ...
                  style={{ background: L.greenBg, border: `1px solid ${L.greenBorder}` }}
                >
                  <div className="flex items-center gap-2">
                    <HippyGhostAvatar seed={1042} className="size-7 rounded-full" />
                    <div>
                      <div className="text-sm font-medium" style={{ color: L.text }}>
                        {t("twoAgents.mockup.personaName")}
                      </div>
                      <div className="font-IBMPlexMono text-xs" style={{ color: L.textMuted }}>
                  ...
                  <div className="space-y-2">
                    {(["suppQ1", "suppQ2"] as const).map((key, i) => (
                      <div key={key} className="flex items-start gap-2">
                        <span
                          className="font-IBMPlexMono text-xs rounded-full w-4 h-4 grid place-items-center shrink-0 mt-0.5"
                          style={{ background: L.border, color: L.textMuted }}
                        >
                          {i + 1}
                        </span>
