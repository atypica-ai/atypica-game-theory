...
const SIGNAL_SNAPSHOTS: Block[][] = [
  [
    { id: "clean", label: "Clean Beauty", weight: 5 },
    { id: "price", label: "Price Sensitivity", weight: 3 },
    { id: "kol", label: "KOL Mentions", weight: 2 },
    { id: "brand", label: "Brand Recall", weight: 4 },
    { id: "sustain", label: "Sustainability", weight: 2 },
    { id: "peer", label: "Peer Reviews", weight: 3 },
    { id: "pack", label: "Packaging", weight: 1 },
  ],
  [
    { id: "clean", label: "Clean Beauty", weight: 3 },
    { id: "price", label: "Price Sensitivity", weight: 5 },
    { id: "kol", label: "KOL Mentions", weight: 4 },
    { id: "brand", label: "Brand Recall", weight: 2 },
    { id: "sustain", label: "Sustainability", weight: 3 },
    { id: "peer", label: "Peer Reviews", weight: 2 },
    { id: "pack", label: "Packaging", weight: 2 },
  ],
  [
    { id: "clean", label: "Clean Beauty", weight: 4 },
    { id: "price", label: "Price Sensitivity", weight: 2 },
    { id: "kol", label: "KOL Mentions", weight: 5 },
    { id: "brand", label: "Brand Recall", weight: 3 },
    { id: "sustain", label: "Sustainability", weight: 4 },
    { id: "peer", label: "Peer Reviews", weight: 1 },
    { id: "pack", label: "Packaging", weight: 3 },
  ],
];
...
function SignalMockup() {
  ...
  useEffect(() => {
    const id = setInterval(() => setTick((t) => (t + 1) % SIGNAL_SNAPSHOTS.length), 2500);
    return () => clearInterval(id);
  }, []);

  const snapshot = SIGNAL_SNAPSHOTS[tick];
  // Layout in percentage coordinates (0-100)
  const rects = useMemo(() => layoutTreemap(snapshot, 100, 100), [snapshot]);
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-2 left-3 right-3 flex items-center gap-2 z-10">
        ...
        />
        <span className="font-IBMPlexMono text-[10px] text-zinc-500">SIGNAL MAP</span>
...
const DEEP_SCREENS = [
      ...
      <div className="p-4 flex flex-col gap-3 h-full justify-center">
        <div className="border border-[rgba(147,197,253,0.2)] bg-[rgba(147,197,253,0.04)] p-3">
          ...
          <span className="text-xs text-zinc-300 leading-relaxed">
            How do Gen-Z consumers perceive clean beauty brands?
          </span>
        </div>
        <motion.div
  ...
  },
  {
    ...
    content: (
      <div className="p-4 flex flex-col gap-2 h-full justify-center">
        <span className="font-IBMPlexMono text-[10px] text-zinc-500">STUDY PLAN</span>
        {["Scout 3 platforms for signals", "Interview 6 AI personas", "Generate insight report"].map(
          (step, i) => (
            <motion.div
              key={step}
  ...
  },
  {
    ...
    content: (
      <div className="p-4 flex flex-col gap-2 h-full justify-center">
        <span className="font-IBMPlexMono text-[10px] text-zinc-500">EXECUTING</span>
        {[
          { step: "Scout", status: "done" },
          { step: "Interview ×6", status: "done" },
...
function DeepMockup() {
  ...
  useEffect(() => {
    const id = setInterval(() => setScreen((s) => (s + 1) % DEEP_SCREENS.length), 3000);
    return () => clearInterval(id);
  }, []);
...
const LIVE_MESSAGES = [
  { role: "ai", text: "What does 'clean beauty' mean to you personally?" },
  { role: "user", text: "For me it's about transparency — knowing what's actually in the product." },
  { role: "ai", text: "When did you first start paying attention to ingredients?" },
] as const;
...
function LiveMockup() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      timers.push(setTimeout(run, LIVE_MESSAGES.length * 2800 + 2000));
    };
    run();
    return () => timers.forEach(clearTimeout);
  }, []);

  const msg = LIVE_MESSAGES[msgIndex];
