...
export const SearchParamsConfig = {
  ...
  adminOnly: createParamConfig.boolean(false),
  viewMode: {
    defaultValue: "actions" as "actions" | "onboarding",
    serialize: (value: "actions" | "onboarding") => value,
    deserialize: (value: string | null) => {
      return value === "onboarding" ? "onboarding" : "actions";
    },
  },
} as const;
...
export function UsersPageClient({ initialSearchParams }: UsersPageClientProps) {
  ...
  const {
    values: { page: currentPage, search: searchQuery, adminOnly, viewMode },
    setParam,
    setParams,
  } = useListQueryParams<UsersSearchParams>({
    params: {
      page: createParamConfig.number(1),
      search: createParamConfig.string(""),
      adminOnly: createParamConfig.boolean(false),
      ...
      <div className="mb-6">
        ...
        <div className="mt-4 flex items-center space-x-2">
          <input
            ...
            checked={adminOnly}
            onChange={(e) => {
              setParams({ adminOnly: e.target.checked, page: 1 }); // Reset to first page when toggling filter
            }}
        ...
        <div className="mt-4">
          <RadioGroup
            ...
            onValueChange={(value: string) =>
              setParam("viewMode", value as "actions" | "onboarding")
            }
