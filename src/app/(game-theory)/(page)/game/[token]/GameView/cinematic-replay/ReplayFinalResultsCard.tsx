...
import { Trophy, Cpu, User, RotateCcw, Share2 } from "lucide-react";
...
interface ReplayFinalResultsCardProps {
...
export function ReplayFinalResultsCard({
  participants,
  cumulativeScores,
  winners,
  isFullTie,
  onRestart,
}: ReplayFinalResultsCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Action buttons */}
      <div className="mt-5 sm:mt-8 flex gap-3">
        <button
          onClick={onRestart}
          className="btn-lab flex-1 flex items-center justify-center gap-2 py-4"
        >
          <RotateCcw size={18} /> WATCH AGAIN
        </button>
        <button
          onClick={handleShareReplay}
          className="flex items-center justify-center gap-2 px-6 py-4 rounded-md border transition-colors"
          style={{
            borderColor: "var(--gt-border)",
            color: copied ? "var(--gt-pos)" : "var(--gt-t2)",
