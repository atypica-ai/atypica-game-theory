...
export function HumanGameView({ initialData, token }: { initialData: GameSessionDetail; token: string }) {
  ...
  }, [gameState.completedRounds.length, gameState.activeRound, isCompleted, gameState.completedRounds]);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ background: "var(--gt-bg)" }}>
      ...
      >
        <div className="mx-auto flex items-center justify-between h-[60px] px-8" style={{ maxWidth: "1200px" }}>
          <div className="flex items-center gap-2">
            <Link href="/" className="text-[13px] transition-colors hover:underline" style={{ color: "var(--gt-t3)", fontFamily: "IBMPlexMono, monospace" }}>
              Game Theory Lab
            </Link>
            <span className="text-[13px]" style={{ color: "var(--gt-t4)" }}>/</span>
            <span className="text-[15px] font-[600]" style={{ color: "var(--gt-t1)", letterSpacing: "var(--gt-tracking-tight)" }}>
              {formatGameTypeName(gameType)}
            </span>
            {!isPending && currentRoundNumber > 0 && (
              <>
                <span className="text-[13px]" style={{ color: "var(--gt-t4)" }}>/</span>
                <span className="inline-flex items-center px-2.5 py-0.5 text-[12px] font-[500] border" style={{ borderRadius: "9999px", color: "var(--gt-t2)", borderColor: "var(--gt-border-md)", fontFamily: "IBMPlexMono, monospace" }}>
          ...
          <div className="flex items-center gap-4">
            {isCompleted && (
              <button
                onClick={() => { if (typeof window !== "undefined") void navigator.clipboard.writeText(`${window.location.origin}/game/${token}/replay`); }}
            ...
            <div className="flex items-center gap-2 px-2.5 py-1 border" style={{ borderRadius: "9999px", borderColor: isCompleted ? "var(--gt-border-md)" : "var(--gt-blue-border)", background: isCompleted ? "transparent" : "var(--gt-blue-bg)" }}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: isCompleted ? "var(--gt-t4)" : isPending ? "var(--gt-t4)" : "var(--gt-blue)", animation: !isCompleted && !isPending ? "pulse 2s infinite" : undefined }} />
              <span className="text-[12px] font-[500]" style={{ color: isCompleted ? "var(--gt-t3)" : isPending ? "var(--gt-t3)" : "var(--gt-blue)", fontFamily: "IBMPlexMono, monospace", letterSpacing: "0.06em" }}>
      ...
      {/* ── Result banner ───────────────────────────────────────────────────── */}
      {bannerData && (
        <div className="shrink-0 h-8 flex items-center justify-center gap-4 border-b" style={{ borderColor: "var(--gt-border)", background: `${bannerData.color}0d` }}>
          <div className="flex-1 h-px max-w-16" style={{ backgroundColor: `${bannerData.color}30` }} />
          <span className="text-[12px] font-[600]" style={{ color: bannerData.color, letterSpacing: "var(--gt-tracking-tight)" }}>{bannerData.text}</span>
          <div className="flex-1 h-px max-w-16" style={{ backgroundColor: `${bannerData.color}30` }} />
        </div>
      )}
      ...
      {navRounds.length > 0 && (
        <div className="shrink-0 h-11 border-b flex items-stretch" style={{ borderColor: "var(--gt-border)", background: "var(--gt-surface)" }}>
          <button
            onClick={() => {
              if (!canGoPrev) return;
              if (showResults) {
                const last = gameState.completedRounds.at(-1);
                if (last !== undefined) dispatchRoundNav({ type: "SELECT_ROUND", roundId: last });
              } else {
                dispatchRoundNav({ type: "SELECT_ROUND", roundId: navRounds[effectiveIdx - 1].roundId });
              }
            }}
            disabled={!canGoPrev}
            className="shrink-0 px-4 h-full flex items-center text-[13px] transition-colors border-r"
            style={{ color: canGoPrev ? "var(--gt-t2)" : "var(--gt-border-md)", borderColor: "var(--gt-border)", fontFamily: "IBMPlexMono, monospace", cursor: canGoPrev ? "pointer" : "default" }}
          >
            ←
          </button>
          ...
          <div className="flex-1 flex items-stretch overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {gameState.activeRound !== null && (
              <button
                onClick={() => dispatchRoundNav({ type: "SELECT_ROUND", roundId: null })}
                className="flex items-center gap-1.5 px-5 h-full text-[13px] font-[500] border-b-2 shrink-0 transition-colors"
                style={{ borderBottomColor: roundNav.manualRoundId === null && !isCompleted ? "var(--gt-blue)" : "transparent", color: roundNav.manualRoundId === null && !isCompleted ? "var(--gt-blue)" : "var(--gt-t3)", fontFamily: "IBMPlexMono, monospace" }}
              >
            ...
            {navRounds.map(({ roundId, isLive }) => {
              ...
              return (
                <RoundPill
                  ...
                  roundId={roundId}
                  payoffSum={payoffSum}
                  isViewing={roundNav.manualRoundId === roundId}
                  isLive={isLive}
                  onClick={() => {
                    if (isLive) dispatchRoundNav({ type: "SELECT_ROUND", roundId: null });
                    else dispatchRoundNav({ type: "SELECT_ROUND", roundId: roundNav.manualRoundId === roundId ? null : roundId });
                  }}
                />
              );
            })}
            ...
            {isCompleted && (
              <button
                onClick={() => dispatchRoundNav({ type: "SELECT_ROUND", roundId: null })}
                className="flex items-center gap-1.5 px-5 h-full text-[13px] font-[500] border-b-2 shrink-0 transition-colors"
                style={{ borderBottomColor: showResults ? "var(--gt-pos)" : "transparent", color: showResults ? "var(--gt-pos)" : "var(--gt-t3)", fontFamily: "IBMPlexMono, monospace" }}
              >
                Results
              </button>
            )}
          ...
          <button
            onClick={() => {
              ...
              if (effectiveIdx === navRounds.length - 1 && isCompleted) {
                dispatchRoundNav({ type: "SELECT_ROUND", roundId: null });
              } else {
                const next = navRounds[effectiveIdx + 1];
                dispatchRoundNav({ type: "SELECT_ROUND", roundId: next.isLive ? null : next.roundId });
              }
            }}
            disabled={!canGoNext}
            className="shrink-0 px-4 h-full flex items-center text-[13px] transition-colors border-l"
            style={{ color: canGoNext ? "var(--gt-t2)" : "var(--gt-border-md)", borderColor: "var(--gt-border)", fontFamily: "IBMPlexMono, monospace", cursor: canGoNext ? "pointer" : "default" }}
          >
            →
          </button>
        </div>
