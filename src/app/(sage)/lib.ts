...
const contentProcessingSchema = z.object({
  ...
  keyPoints: z
    .array(z.string())
    .describe("Key knowledge points identified in the content"),
  contentQuality: z.object({
    completeness: z
      .number()
      .min(0)
      .max(1)
      .describe("Content completeness score (0-1)"),
  ...
  ambiguousAreas: z
    .array(z.string())
    .optional()
    .describe("Areas that need clarification"),
});
...
const memoryExtractionSchema = z.object({
  memories: z.array(
    z.object({
      content: z
        .string()
        .describe("Detailed memory content with full context"),
      category: z.string().describe("Topic category for this memory"),
      tags: z.array(z.string()).describe("Related tags (3-5 tags)"),
      importance: z
        .enum(["high", "medium", "low"])
        .describe("Importance level of this knowledge"),
      keyTakeaway: z
        .string()
        .describe("One-sentence summary of this memory"),
    })
  ),
...
const knowledgeAnalysisSchema = z.object({
  overallScore: z
    ...
    .min(0)
    .max(100)
    .describe("Overall knowledge completeness score"),
  dimensions: z.array(
    z.object({
  ...
  shouldInterview: z
    .boolean()
    .describe("Whether supplementary interview is recommended"),
});
 ...
 */
export async function generateSageEmbedding(
  memoryDocument: string
): Promise<number[]> {
 ...
 */
export async function getSageByToken(token: string) {
