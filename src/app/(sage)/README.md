...
Step 3: 知识分析
  ...
  2. createKnowledgeGaps
     - 存入独立 KnowledgeGap 表
     - sourceType: "analysis"
     - status: "pending"

Step 4: 生成向量嵌入
  - generateSageEmbedding (Jina Embeddings v3)
  - 1024 维向量
  - 存储为 halfvec 类型
```

**优化说明**:
- ✅ **简化**: 移除了 extractMemories 步骤，Memory Document 已足够
- 💰 **节省**: AI 调用从 4 次减少到 3 次
- ⚡ **更快**: 减少一次 Claude Sonnet-4 调用
...
- 如果分析结果已存在，跳过
- 防止重复创建 KnowledgeGap（检查 sourceType="analysis" 的数量）

**Step 4 - 生成向量**:
- 检查条件：通过 SQL 查询 embedding 字段是否为 null
- 如果已存在 embedding，跳过
...
12. ✅ 对话分析识别知识空白
13. ✅ 公开专家页面
14. ✅ 简化处理流程（移除 extractMemories）
15. ✅ 断点续传机制（Resume from Failed Step）
16. ✅ 重试功能（前端 + 后端）
