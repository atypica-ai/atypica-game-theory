...
import type { CreateSageInput, SageExtra, UpdateSageInput } from "./types";
...
> {
  return withAuth(async (user) => {
    try {
      ...
      // Get knowledge analysis from extra field
      const analysis = (sage.extra as SageExtra).knowledgeAnalysis;

      // Get knowledge gaps for interview planning
      const knowledgeGaps = analysis?.dimensions
        ?.filter((d) => d.level === "low" || d.level === "medium")
        .map((d) => d.name) || ["General knowledge enhancement"];

      // Generate interview plan if we have knowledge gaps from analysis
      let interviewPlan;
      if (sage.memoryDocument && analysis) {
        const { generateInterviewPlan } = await import("./lib");
        const fullKnowledgeGaps =
          analysis.knowledgeGaps ||
          analysis.dimensions
            ?.filter((d) => d.level === "low" || d.level === "medium")
            .map((d) => ({
              area: d.name,
              severity: d.level === "low" ? ("critical" as const) : ("important" as const),
              description: d.assessment,
              impact: d.assessment,
              suggestedQuestions: d.improvementSuggestions,
            })) ||
          [];

        if (fullKnowledgeGaps.length > 0) {
      ...
      const interview = await prisma.sageInterview.create({
        data: {
          ...
          purpose:
            ...
            (locale === "zh-CN"
              ? "补充专家知识，填补知识空白"
              : "Supplement expert knowledge and fill knowledge gaps"),
          focusAreas: interviewPlan?.focusAreas || knowledgeGaps,
          status: "ongoing",
          progress: 0,
          extra: interviewPlan ? { interviewPlan } : {},
        },
      });
