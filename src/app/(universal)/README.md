...
Lead agent 通过 `createStudySubAgent` 工具创建 sub-agent 执行研究任务。
...
**Sub-agent**（`createStudySubAgent/index.ts`）：
```
config.systemPrompt  // study 基础提示词
modePrompt           // 模式执行要求（study/flexible/panel）— createStudySubAgent/prompt.ts
...
- **工具集**：`src/app/(universal)/tools/`
  - `createStudySubAgent/` - Sub-agent 创建和管理
