...
type SessionWithProject = InterviewSession & {
  project: Pick<InterviewExpertProject, "id" | "title" | "description" | "type" | "objectives">;
};

export function SharedInterviewPageClient({ session }: { session: SessionWithProject }) {
  ...
  const { messages, input, handleInputChange, handleSubmit, append, isLoading, error } = useChat({
    ...
    api: "/api/chat/interviewExpert/shared",
  ...
  useEffect(() => {
    ...
    if (messages.length === 0 && !session.userChatId) {
      append({
        role: "system",
        content: "Starting shared interview session.",
      });
    }
