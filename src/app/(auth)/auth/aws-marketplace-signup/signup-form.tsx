...
export function AWSMarketplaceSignupForm({
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Link AWS Marketplace customer
      const linkResponse = await fetch("/api/aws-marketplace/link-customer", {
        method: "POST",
      ...
      router.push(`/auth/verify?email=${email}&callbackUrl=${encodeURIComponent("/")}`);
    } catch (error) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-3 text-sm text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
        You are registering through AWS Marketplace. Your subscription will be managed through
        AWS.
      </div>
        ...
        {password && (
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {passwordStrength.level === 3 && <ShieldCheck size={16} className="text-green-500" />}
