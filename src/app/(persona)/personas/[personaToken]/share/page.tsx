...
async function PersonaDetailSharePage({ personaToken }: { personaToken: string }) {
  const result = await fetchPersonaWithDetails(personaToken);
  if (!result.success) {
  ...
  const { persona } = result.data;
  return <PersonaDetailClient persona={persona} analysis={null} personaImportId={null} />;
}

export default async function PersonaDetailSharePageWithLoading({
  params,
}: {
  params: Promise<{ personaToken: string }>;
}) {
  ...
  return (
    <Suspense fallback={<PageLoadingFallback />}>
      <PersonaDetailSharePage personaToken={personaToken} />
    </Suspense>
  );
}
