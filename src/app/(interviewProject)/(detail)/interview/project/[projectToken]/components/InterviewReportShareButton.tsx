"use client";
import { generateInterviewReportPDFAction } from "@/app/(interviewProject)/artifacts/actions";
import {
  AlertDialog,
  AlertDialogCancel,
  AlertDialogContent,
...
export function InterviewReportShareButton({
  ...
  const publicReportUrl = `/artifacts/interview-report/${reportToken}/share`;
  ...
  const handleDownloadPDF = useCallback(() => {
    startTransition(async () => {
      try {
        ...
        const { pdfUrl, fileName } = await generateInterviewReportPDFAction(reportToken);
