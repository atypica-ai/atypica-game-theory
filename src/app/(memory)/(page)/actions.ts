...
export async function createMemoryBuilderChat({
  mode,
}: {
  mode: "team" | "user";
  ...
  // Team mode: verify team ownership
  if (mode === "team") {
  ...
  try {
    const userChat = await createUserChat({
      ...
      title: mode === "team" ? "Team Context Interview" : "Personal Context Interview",
      kind: "misc",
    });

    return { success: true, data: { userChatToken: userChat.token } };
  } catch (error) {
    console.error("Failed to create memory builder chat:", error);
    return { success: false, message: "Failed to create chat", code: "internal_server_error" };
  }
}
