...
import { TContextBuilderUITools } from "@/app/(memory)/user/memory-builder/tools/types";
...
export function ContextBuilderChatClient({
  ...
  const useChatHelpers = useChat({
    ...
    transport: new DefaultChatTransport<CONTEXT_BUILDER_UI_MESSAGE>({
      api: "/api/user-memory-builder/chat",
  ...
  const t = useTranslations("User.MemoryBuilder.chatPage");
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (editedMemory !== memory) {
      try {
        const result = await saveUserCoreMemory(
          editedMemory,
          "Edited from personal context builder",
        );
        if (!result.success) {
