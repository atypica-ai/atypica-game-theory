...
export const contextBuilderTools = ({ locale, teamId }: { locale: Locale; teamId: number }) => ({
  endInterview: tool({
    description:
      ...
      try {
        ...
        await prisma.memory.create({
          data: {
            ...
            core: memory,
            working: [],
            changeNotes: "Initial memory from context builder interview",
          },
        });

        console.log(`✅ Memory saved to database (team: ${teamId}, version: ${nextVersion})`);
      } catch (error) {
