...
## 6. 管理 Persona
在面板详情页（`/panel/{panelId}`）下半部分的 Persona 列表中：
- **查看详情**：点击 Persona 卡片，弹出完整属性（角色、标签、背景等）。
- **添加 Persona**：点击「添加」按钮，打开搜索弹窗选择后加入 Panel（自动去重）。
- **移除 Persona**：鼠标悬停卡片右上角出现「×」，点击后确认即可移除。

## 7. 页面结构
- 面板详情页（`/panel/{panelId}`）：
  - 上半部分是该 Panel 下的研究项目列表
  - 下半部分是 Persona 列表与管理（查看详情、添加、移除）
