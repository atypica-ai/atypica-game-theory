...
- `extra.runId` 非空表示后台 Agent 正在运行（managed runtime）
...
- 使用 `UserChat.extra.runId`（通过 `startManagedRun()`）标识运行状态
- `runId` 非空 = Agent 正在跑；`managed.cleanup()` 清除 runId
- 定义在 `src/lib/userChat/runtime.ts`，所有 agent 统一使用
- Panel 前端通过 `extra.runId` 判断初始状态，`fetchProjectProgress()` 轮询实时更新
- Interview 的 `interviewUserChat` 也有自己的 managed run
