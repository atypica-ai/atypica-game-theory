...
**运行状态标识（Managed Runtime）**：
- 新系统使用 `UserChat.extra.runId`（通过 `startManagedRun()`）替代旧的 `backgroundToken`
- `runId` 非空 = Agent 正在跑；`managed.cleanup()` 清除 runId
- 定义在 `src/lib/userChat/runtime.ts`，sub-tools（interview、scout 等）统一使用
- **注意**：Panel 前端目前同时看两个信号：
  - `project.backgroundToken`（旧机制，仍在 DB schema 中）用于初始状态判断
  - `fetchProjectProgress()` 轮询（新机制）用于实时更新
- Interview 的 `interviewUserChat` 也有自己的 managed run，`saveMessage()` 校验 `extra.runId`
...
6.                     Agent 完成                     managed.cleanup()               绿色脉冲消失
                                                     (清除 extra.runId)
```
...
single(persona):
  ...
  runInterview()
    └→ startManagedRun() → 设置 extra.runId（标识运行中）
    └→ 监听父级 abortSignal → abort 时自动 managed.cleanup()
    └→ while(true) 循环:
        chatWithPersona(messages)
          └→ persona agent 回复（可使用社交媒体搜索工具）
          └→ 保存消息到 interviewUserChat（校验 runId 一致性）
    ...
    └→ managed.cleanup()（清除 extra.runId）
...
UserChat (研究项目)
  ...
  ├── extra.runId: string | ""          → 非空表示 Agent 运行中（managed runtime）
  └── backgroundToken: string | null    → 旧字段，部分代码仍在读取
...
| `src/lib/userChat/runtime.ts` | Managed runtime：startManagedRun / cleanup |
