...
```mermaid
graph LR
    A[User needs] --> B[Plan Mode Strategic layer]
    B --> C{Auto-determine}
    C -->|kind = insights| D[Study Agent]
    C -->|kind = productRnD| E[Product R&D Agent]
    C -->|kind = fastInsight| F[Fast Insight Agent]

    D --> G[planStudy Tactical layer]
    E --> H[Internal planning]
    F --> I[Internal planning]

    G --> J[Execution details]
    H --> J
    I --> J
```
...
sequenceDiagram
    ...
    User->>Frontend: "I want to understand young people's views on sparkling coffee"
    Frontend->>Backend: POST /api/chat/study
    Backend->>AI: Enter Plan Mode

    loop Intent clarification
        AI->>Backend: Clarifying question
    ...
    AI->>Backend: Call makeStudyPlan tool
    Note over Backend: Tool has no execute, doesn't run
    Backend->>Frontend: Stream return tool-call
    Frontend->>User: Display complete plan + confirm button

    User->>Frontend: Click "Confirm execution"
    Frontend->>Backend: saveAnalystFromPlan(analystId, kind, role, topic)
    Note over Backend: Save analyst.kind
    Frontend->>Backend: addToolResult(confirmed=true)

    Backend->>AI: Reload userChat
    Note over Backend: analyst.kind is now set
    Backend->>AI: Route to execution Agent
    AI->>Backend: Start executing research
```
