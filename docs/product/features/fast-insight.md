...
> - 分析设计师工作流变化
> - 调研行业案例和数据
> - 报告结构：技术趋势 → 影响分析 → 机会与威胁 → 行动建议
> - 播客大纲：开场（1 分钟）→ 工具介绍（5 分钟）→ 案例分析（7 分钟）→ 建议（2 分钟）

### 第 2 步：自动搜索与分析（2-3 小时）
...
嘉宾：举个例子，以前做 Logo 提案，我需要手绘草稿、电脑精修，可能要 2-3 天才能给客户看 5 个方案。现在我用 Midjourney，半天就能生成 50 个方向...
...
| **研究深度** | 基于网络搜索和二手信息 | 支持 Interview/Discussion/Scout 一手调研 |
| **输出形式** | 报告 + 播客脚本（标配） | 报告（播客可选）|
| **适用场景** | 快速输出、追热点、系列内容 | 复杂研究、深度洞察、产品决策 |
| **信息来源** | 网络搜索为主 | 可接入 AI 访谈、社交观察、MCP 数据源 |
| **研究方法** | 桌面研究 | Interview、Discussion、Scout、桌面研究 |
...
**Token 消耗**：
- 单次 Fast Insight：约 50,000-100,000 tokens
- 订阅计划包含足够 token 配额
